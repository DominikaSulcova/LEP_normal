...
```{r,message=FALSE,warning=FALSE}
# read .csv files
data_all <- read_csv(file.path(wd_path, paste0('AperiodicPFC_export_sources_all.csv')))
data2exclude <- read_csv2(file.path(wd_path, 'AperiodicPFC_data2exclude.csv'))
data2exclude <- data2exclude %>%
  filter(!is.na(subject))
source_labels <- read_csv(file.path(wd_path, 'AperiodicPFC_source_labels.csv'))
...
# prepare outcome df
outcome_APC <- data.frame(region_n = numeric(),
...
# loop through regions
  for (reg in regions) {
    ...
    var <- paste0('exponent', as.character(reg))
    data_APC <- data_all %>% 
      filter(component == !!'N2') %>%
      select(c('subject', 'trial', 'area', var)) 
      ...
      outcome_APC <- outcome_APC %>%
        add_row(region_n = reg,
                region_label = source_labels$label[reg], 
                mean = output$summary_stats$mean,
                sd = output$summary_stats$sd,
                sem = output$summary_stats$sem)
...
# plot mean values
outcome_APC_ordered <- outcome_APC %>%
  arrange(desc(mean)) %>%
  mutate(region_label = factor(region_label, levels = region_label))
barplot <- ggplot(outcome_APC_ordered, aes(x = region_label, y = mean)) +
  ...
  ylab("Mean exponent value") 
plot(barplot)
rm(regions, reg, var, data_APC, output)
```
...
# prepare outcome df
outcome_N1_amplitude <- data.frame(region_n = numeric(),
                         region_label = character(),
                         F_value = numeric(), 
                         p_value = numeric(),
...
# loop through regions
  for (reg in regions) {
    ...
    data <- data_all %>%
      filter(component == "N1") %>% 
      select(c('subject', 'component', 'area', 'trial', var, 'amplitude')) %>% 
      rename(exponent = var)
    ...
    # evaluate the model
    model <- lmer(amplitude_log ~ exponent + area + (1 | subject), data = data)
    results <- anova(model)
    
    # append to the outcome
   if(results["exponent", "Pr(>F)"] < 0.05) {
      signif <- '*'
    } else {
      signif <- ''
    }
    outcome_N1_amplitude <- outcome_N1_amplitude %>%
      add_row(region_n = reg,
              region_label = source_labels$label[reg], 
              F_value = results["exponent", "F value"],
              p_value = results["exponent", "Pr(>F)"],
              significant = signif)
  }
```
...
# prepare outcome df
outcome_N1_latency <- data.frame(region_n = numeric(),
                         region_label = character(),
                         F_value = numeric(), 
                         p_value = numeric(),
                         significant = character(),
...
# loop through regions
  for (reg in regions) {
    ...
    data <- data_all %>%
      filter(component == "N1") %>% 
      select(c('subject', 'component', 'area', 'trial', var, 'latency')) %>% 
      rename(exponent = var)
    
    # evaluate the model
    model <- lmer(latency ~ exponent + area + (1 | subject), data = data)
    results <- anova(model)
   ...
   if(results["exponent", "Pr(>F)"] < 0.05) {
    ...
    outcome_N1_latency <- outcome_N1_latency %>%
      add_row(region_n = reg,
              region_label = source_labels$label[reg], 
              F_value = results["exponent", "F value"],
              p_value = results["exponent", "Pr(>F)"],
...
# prepare outcome df
outcome_N2_amplitude <- data.frame(region_n = numeric(),
                         region_label = character(),
                         F_value = numeric(), 
                         p_value = numeric(),
                         significant = character(),
...
# loop through regions
  for (reg in regions) {
   ...
   if(results["exponent", "Pr(>F)"] < 0.05) {
    ...
    outcome_N2_amplitude <- outcome_N2_amplitude %>%
      add_row(region_n = reg,
              region_label = source_labels$label[reg], 
              F_value = results["exponent", "F value"],
              p_value = results["exponent", "Pr(>F)"],
...
# prepare outcome df
outcome_N2_latency <- data.frame(region_n = numeric(),
                         region_label = character(),
                         F_value = numeric(), 
                         p_value = numeric(),
                         significant = character(),
...
# loop through regions
  for (reg in regions) {
   ...
   if(results["exponent", "Pr(>F)"] < 0.05) {
    ...
    outcome_N2_latency <- outcome_N2_latency %>%
      add_row(region_n = reg,
              region_label = source_labels$label[reg], 
              F_value = results["exponent", "F value"],
              p_value = results["exponent", "Pr(>F)"],
...
# prepare outcome df
outcome_P2_amplitude <- data.frame(region_n = numeric(),
                         region_label = character(),
                         F_value = numeric(), 
                         p_value = numeric(),
                         significant = character(),
...
# loop through regions
  for (reg in regions) {
   ...
   if(results["exponent", "Pr(>F)"] < 0.05) {
    ...
    outcome_P2_amplitude <- outcome_P2_amplitude %>%
      add_row(region_n = reg,
              region_label = source_labels$label[reg], 
              F_value = results["exponent", "F value"],
              p_value = results["exponent", "Pr(>F)"],
...
# prepare outcome df
outcome_P2_latency <- data.frame(region_n = numeric(),
                         region_label = character(),
                         F_value = numeric(), 
                         p_value = numeric(),
                         significant = character(),
...
# loop through regions
  for (reg in regions) {
   ...
   if(results["exponent", "Pr(>F)"] < 0.05) {
    ...
    outcome_P2_latency <- outcome_P2_latency %>%
      add_row(region_n = reg,
              region_label = source_labels$label[reg], 
              F_value = results["exponent", "F value"],
              p_value = results["exponent", "Pr(>F)"],
...
# prepare outcome df
outcome_rating <- data.frame(region_n = numeric(),
                         region_label = character(),
                         F_value = numeric(), 
                         p_value = numeric(),
                         significant = character(),
...
# loop through regions
  for (reg in regions) {
    ...
    data <- data_all %>%
      filter(component == "N2") %>% 
      select(c('subject', 'component', 'area', 'trial', var, 'rating', 'rating_norm')) %>% 
      rename(exponent = var)
    
    # evaluate the model
    model <- lmer(rating ~ exponent + area + (1 | subject), data = data)
    results <- anova(model)
...
# pain rating - only hand
```{r}
# define variables
regions <- 1:95

# prepare outcome df
outcome_rating_hand <- data.frame(region_n = numeric(),
                         region_label = character(),
                         F_value = numeric(), 
                         p_value = numeric(),
                         significant = character(),
...
# loop through regions
  for (reg in regions) {
    ...
    data <- data_all %>%
      filter(component == "N2") %>% 
      filter(area == "hand") %>% 
      select(c('subject', 'component', 'trial', var, 'rating', 'rating_norm')) %>% 
      rename(exponent = var)
    
    # evaluate the model
    model <- lmer(rating ~ exponent + (1 | subject), data = data)
    results <- anova(model)
   ...
   if(results["exponent", "Pr(>F)"] < 0.05) {
    ...
    outcome_rating_hand <- outcome_rating_hand %>%
      add_row(region_n = reg,
              region_label = source_labels$label[reg], 
              F_value = results["exponent", "F value"],
              p_value = results["exponent", "Pr(>F)"],
...
# pain rating - only foot
```{r}
# define variables
regions <- 1:95

# prepare outcome df
outcome_rating_foot <- data.frame(region_n = numeric(),
                         region_label = character(),
                         F_value = numeric(), 
                         p_value = numeric(),
                         significant = character(),
...
# loop through regions
  for (reg in regions) {
   ...
   if(results["exponent", "Pr(>F)"] < 0.05) {
    ...
    outcome_rating_foot <- outcome_rating_foot %>%
      add_row(region_n = reg,
              region_label = source_labels$label[reg], 
              F_value = results["exponent", "F value"],
              p_value = results["exponent", "Pr(>F)"],
...
# export for visualization in MATLAB
```{r}
# N1 amplitude
outcome <- outcome_N1_amplitude %>% 
  filter(significant == '*')
write.csv(outcome, "signifsources_N1_amplitude.csv", row.names = FALSE)

# N1 latency
outcome <- outcome_N1_latency %>% 
  filter(significant == '*')
write.csv(outcome, "signifsources_N1_latency.csv", row.names = FALSE)

# N2 amplitude
outcome <- outcome_N2_amplitude %>% 
  filter(significant == '*')
write.csv(outcome, "signifsources_N2_amplitude.csv", row.names = FALSE)

# N2 latency
outcome <- outcome_N2_latency %>% 
  filter(significant == '*')
write.csv(outcome, "signifsources_N2_latency.csv", row.names = FALSE)


# P2 amplitude
outcome <- outcome_P2_amplitude %>% 
  filter(significant == '*')
write.csv(outcome, "signifsources_P2_amplitude.csv", row.names = FALSE)

# P2 latency
outcome <- outcome_P2_latency %>% 
  filter(significant == '*')
write.csv(outcome, "signifsources_P2_latency.csv", row.names = FALSE)

# ratings
outcome <- outcome_rating %>% 
  filter(significant == '*')
write.csv(outcome, "signifsources_rating.csv", row.names = FALSE)
```