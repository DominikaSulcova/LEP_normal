...
# read .csv files
foi <- 'high'
data_all <- read_csv(file.path(wd_path, paste0('AperiodicPFC_export_', foi, '.csv')))
...
# for (ar in areas) {
  for (reg in regions) {
    for (var in variables) {
      ...
      # save the plots as SVG files
      save_plot(paste0(wd_path, '\\figures\\APC_', foi,  '_', var, "_", reg, "_distribution.svg"), output$dist_plot)
      save_plot(paste0(wd_path, '\\figures\\APC_', foi,  '_', var, "_", reg, "_subjects.svg"), output$dot_plot)
    }
  }
# }
print(outcome_APC)
...
# for (ar in areas) {
  for (var in variables) {
      ...
      # display distribution
      print(output$dist_plot_group)
      save_plot(paste0(wd_path, '\\figures\\APC_', foi,  '_', var, "_distribution_all.svg"), output$dist_plot_group)
      
      # display boxplot
      print(output$boxplot_group)
      save_plot(paste0(wd_path, '\\figures\\APC_', foi,  '_', var, "_boxplot_all.svg"), output$boxplot_group)
  }
# }
...
data <- data_all %>%
  filter(component == "N1") %>% 
  select(c('subject', 'age', 'male', 'dominant', 'area', 'region', 'amplitude', 'exponent', 'offset')) 
...
# try computing a separate model for the target region
model_target <- lmer(rating ~ exponent + area + (1 | subject), data = data %>% filter(region == "target"))
anova(model_target)
outliers <- check_model(model_target, data)
...
data_target <- data %>% 
  filter(region == "target")
model_target <- lmer(rating ~ exponent + (1 | subject), data = data_target)
anova(model_target)
outliers <- check_model(model_target, data_target)

data_ctrl <- data%>% 
  filter(region == "ctrl")
model_ctrl <- lmer(rating ~ exponent + (1 | subject), data = data_ctrl)
anova(model_ctrl)
outliers <- check_model(model_ctrl, data_ctrl)
