...
```{r,message=FALSE,warning=FALSE}
# read .csv files
data_all <- read_csv(file.path(wd_path, paste0('AperiodicPFC_export_high.csv')))
...
for (ar in areas) {
  for (reg in regions) {
    for (var in variables) {
      ...
      # save the plots as SVG files
      save_plot(paste0(wd_path, '\\figures\\APC-sensor_', ar, '_', var, "_", reg, "_distribution.svg"), output$dist_plot)
      save_plot(paste0(wd_path, '\\figures\\APC-sensor_', ar, '_', var, "_", reg, "_subjects.svg"), output$dot_plot)
    }
  }
}
print(outcome_APC)
...
for (ar in areas) {
  for (var in variables) {
      ...
      # display distribution
      print(output$dist_plot_group)
      save_plot(paste0(wd_path, '\\figures\\APC-sensor_', ar, '_', var,  "_distribution_all.svg"), output$dist_plot_group)
      
      # display boxplot
      print(output$boxplot_group)
      save_plot(paste0(wd_path, '\\figures\\APC-sensor_', ar, '_', var,  "_boxplot_all.svg"), output$boxplot_group)
  }
}
...
for (var in variables) {
    ...
    # display distribution
    print(output$dist_plot_group)
    save_plot(paste0(wd_path, '\\figures\\APC-sensor_avg_', var,  "_distribution_all.svg"), output$dist_plot_group)
    
    # display boxplot
    print(output$boxplot_group)
    save_plot(paste0(wd_path, '\\figures\\APC-sensor_avg_', var,  "_boxplot_all.svg"), output$boxplot_group)
}
...
model_lateral <- lmer(rating ~ exponent + area + (1 | subject), data = data_lateral)
anova(model_lateral)
outliers <- check_model(model_lateral, data_lateral)

data_medial <- data %>% 
  filter(region == "medial")
model_medial <- lmer(rating ~ exponent + area + (1 | subject), data = data_medial)
anova(model_medial)
